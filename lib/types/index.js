 ...
 */
export function foldAuditMode(events, end = events.length, defaultActive = false) {
    ...
    for (const event of events) {
        if (index >= end)
            break;
        index += 1;
        if (event.type === 'readonly-audit/mode')
            active = event.data.active;
    }
    return active;
}
/** Read the last selected delivery method, or `null` before a choice. */
export function foldAuditDelivery(events) {
    let delivery = null;
    for (const event of events) {
        if (event.type === 'readonly-audit/delivery')
            delivery = event.data.delivery;
    }
    return delivery;
}
/** The latest entry event's saved previous sandbox mode, if any. */
export function foldPreviousSandbox(events) {
    for (let index = events.length - 1; index >= 0; index -= 1) {
        const event = events[index];
        if (event.type === 'readonly-audit/mode' && event.data.active)
            return event.data.previousSandbox;
    }
    return undefined;
}
/** The latest entry event's saved previous approval policy, if any. */
export function foldPreviousApproval(events) {
    for (let index = events.length - 1; index >= 0; index -= 1) {
        const event = events[index];
        if (event.type === 'readonly-audit/mode' && event.data.active)
            return event.data.previousApproval;
    }
    return undefined;
}
...
function isFileMutation(name, args, extraMutatingTools) {
    if (FILE_MUTATOR_TOOLS.has(name) || extraMutatingTools.has(name))
        return true;
...
export class ReadonlyAuditController extends Service {
    ...
    constructor(ctx, config = {}) {
        ...
        ctx.systemPrompt.section({
            ...
            text: (context) => {
                ...
                if (agent === undefined || this.isActive(agent.session) === false)
                    return '';
        ...
        ctx.on('tools/post-execute', async (exec, _result, next) => {
            if (exec.agent !== undefined)
                this.restoreAfterApprovedMutation(exec.agent, exec.token);
            return await next();
        }, true);
    ...
    set(agent, active) {
        ...
        const approval = this.ctx.get('approval');
        if (approval !== undefined)
            approval.setPolicy(agent, previousApproval);
    ...
    command(agent, rawInput) {
        ...
        if (rawInput.trim() === '' || verb === 'status') {
            ...
            if (!state.active)
                return { kind: 'success', text: 'Read-only security audit mode is off.' };
    ...
    async preExecute(exec) {
        ...
        if (exec.name === CHOOSE_DELIVERY_TOOL)
            return { kind: 'allow' };
    ...
    restoreAfterApprovedMutation(agent, token) {
        ...
        if (approved === undefined)
            return;
        if (approved.delete(token) === false)
            return;
        if (approved.size > 0)
            return;
        try {
...
function requireAgent(exec) {
    if (exec.agent === undefined)
        throw new Error('read-only security audit tools require a calling agent');
    return exec.agent;
}
